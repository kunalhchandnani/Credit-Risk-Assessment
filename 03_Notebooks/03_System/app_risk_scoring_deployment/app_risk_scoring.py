...
from execution_script import run_models
...
img_sidebar_path = os.path.join(base_dir, 'img_sidebar.png')
img_title_path = os.path.join(base_dir, 'img_title.png')
pipe_pd_path = os.path.join(base_dir, 'pipe_execution_pd.pickle')

# Check if the required file exists
if not os.path.exists(pipe_pd_path):
    st.error(f"File not found: {pipe_pd_path}")
    st.stop()

# Load images
im_sidebar = Image.open(img_sidebar_path)
im_title = Image.open(img_title_path)

# PAGE SETUP
st.set_page_config(
     page_title='Credit Risk Assessment',
     page_icon=os.path.join(base_dir, 'icon.png'),
     layout='wide',
...
with st.sidebar:
    ...
    with col2:
        form_button = st.button('NEW LOAN FORM')
    with col4:
